...
fn item(p: &mut Parser, wrapper: Option<(Marker, SyntaxKind)>) -> bool {
    ...
    match p.eat() {
        SyntaxKind::Meta | SyntaxKind::Identifier | SyntaxKind::Dot | SyntaxKind::Bar => {}
        ...
        SyntaxKind::Tilde => {
            if !item(p, Some((start, SyntaxKind::Converse))) {
                p.unexpected();
                p.hint("consider using converse indicator `~` before an expression")
            }
        }
        ...
        SyntaxKind::RightParen | SyntaxKind::Action | SyntaxKind::End | SyntaxKind::SemiColon => {
            p.uneat();
            return false;
        }
